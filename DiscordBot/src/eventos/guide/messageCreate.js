...
module.exports = async (client, message) => {
    ...
    if (COMANDO) {
        ...
        if (COMANDO.BOT_PERMISSIONS) {
            if (!message.guild.member.me.permissions.has(COMANDO.BOT_PERMISSIONS)) return message.reply({ content: `❌ **Necesito los siguientes permisos para ejecutar este comando.** \n${COMANDO.BOT_PERMISSIONS.map(PERMISO => `\`${PERMISO}\``).join('. ')}` });
        }
        ...
        try {
            ...
            message.reply({ content: `**Ha ocurrido un error a la hora de ejecutar el comando** \r*Mira la consola para más detalles*` });
            console.log(e)
            return;
        }
    }
